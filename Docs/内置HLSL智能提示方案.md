...
用正则从用户 HLSL 代码中提取 5 类符号：

| 符号类型 | 匹配模式 | 示例 |
|---------|---------|------|
| 函数 | `returnType funcName(params) {` | `float3 myFunc(float2 uv, float t) {` |
| 参数 | 从函数声明的参数列表中提取 | `float2 uv`、`inout float t` |
...
**参数作用域管理：** 函数参数通过 `scopeStartLine`/`scopeEndLine` 限定可见范围（函数声明行到函数体结束行），仅在函数体内可见。大括号匹配跳过注释和字符串字面量。
...
3. **正则扫描的局限**：无法匹配数组声明（`float4 colors[4];`）、数组参数（`float arr[4]`）、跨行块注释中的假匹配等边界情况
4. **补全列表可能有重名**：如果用户定义了和内置函数同名的符号，补全列表会出现两个
5. **多行函数参数行号**：多行函数声明中，参数的 line/col 指向函数声明首行而非参数实际所在行
